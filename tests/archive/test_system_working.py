...
def test_batches_endpoint():
    ...
    try:
        ...
        if response.status_code == 200:
            result = response.json()
            print(f"   ✅ Batches retrieved: {len(result.get('batches', []))} batches")
            return True
        else:
            print(f"   ❌ Batches failed: {response.text}")
            return False
    except Exception as e:
        print(f"   ❌ Error: {e}")
        return False
...
def main():
    ...
    if tests_passed == total_tests:
        ...
        print(f"\n⚠️  {total_tests - tests_passed} test(s) failed but core system is working.")
        print("Check API endpoints and database connection.")
        return 1
    else:
        print("\n❌ System has major issues. Check backend logs.")
        return 1
