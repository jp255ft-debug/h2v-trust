...
try:
    ...
    if response.status_code == 200:
        ...
        print("   This might be due to duplicate token_id in mock mode")
    else:
...
try:
    ...
    if response.status_code == 200:
        ...
        print("   ✅ Batches endpoint working!")
    else:
        print(f"   Response: {response.text}")
except Exception as e:
    print(f"   ❌ Error: {e}")
...
try:
    ...
    if response.status_code == 200:
        ...
        if certificates:
            print(f"   First certificate: {certificates[0]}")
        print("   ✅ Certificates endpoint working!")
    else:
        print(f"   Response: {response.text}")
except Exception as e:
    print(f"   ❌ Error: {e}")
